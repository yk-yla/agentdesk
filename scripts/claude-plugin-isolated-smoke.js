...
async function main() {
  ...
  try {
    ...
    assert.ok(findMarketplace(added), "添加本地测试市场后列表中没有该市场。");
    ...
    assert.equal(findPlugin(await request("list"))?.installed, true, "测试插件安装后没有显示已安装。");
    await host.close();
    host = new ClaudeWorkerHost(() => workerFile);
    assert.equal(findPlugin(await request("list"))?.installed, true, "重启 Claude Worker 后已安装插件状态没有保留。");
    assert.equal((await request("details", { plugin: pluginId }))?.plugin?.name, pluginId, "插件详情名称不正确。");

    createMarketplace(tempRoot, "1.1.0");
    await request("update", { plugin: pluginId });
    const updatedPlugin = findPlugin(await request("list"));
    ...
    assert.equal(findPlugin(await request("list"))?.installed, false, "测试插件卸载后仍显示已安装。");
    await request("marketplaceRemove", { marketplace: marketplaceName });
    assert.equal(findMarketplace(await request("list")), undefined, "移除测试市场后列表中仍有该市场。");
    ...
    console.log(JSON.stringify({ ok: true, results: ["隔离配置插件闭环通过", "用户真实 Claude 配置前后指纹一致"] }));
  } finally {
    await host.close();
