...
function HistorySidebarBase({ viewModel, actions }: HistorySidebarProps) {
  const [search, setSearch] = useState("");
  const [visibleCount, setVisibleCount] = useState(HISTORY_BATCH_SIZE);
  const [view, setView] = useState<"directory" | "favorites">("directory");
  ...
  const selected = view === "favorites" ? viewModel.favoriteHistory : viewModel.historySearchResults || viewModel.directoryHistory;
  const filtered = useMemo(() => filterSidebarHistory({ entries: selected, provider, query: search, applyTitleFilter: view !== "directory" || !viewModel.historySearchResults }), [provider, search, selected, view, viewModel.historySearchResults]);
  const visible = useMemo(() => filtered.slice(0, visibleCount), [filtered, visibleCount]);

  const selectView = (next: "directory" | "favorites") => {
    setView(next);
    if (viewModel.historySearchResults || viewModel.historySearchLoading) {
      setSearch("");
      actions.onSearchHistory("");
    }
  };

  return <>
    <div className={`history-search ${viewModel.historySearchResults ? "global" : ""}`}><input aria-label={view === "favorites" ? "搜索收藏会话标题" : "搜索当前目录会话标题"} value={search} onChange={(event) => { setSearch(event.target.value); if (viewModel.historySearchResults || viewModel.historySearchLoading) actions.onSearchHistory(""); }} placeholder={view === "favorites" ? "搜索收藏标题" : "搜索当前目录标题"} />{viewModel.historySearchLoading && view === "directory" ? <small>搜索中</small> : null}{view === "directory" ? <button type="button" className={`bare-button history-content-search ${viewModel.historySearchResults ? "active" : ""}`} onClick={() => actions.onSearchHistory(search)} disabled={!search.trim() || viewModel.historySearchLoading} title="搜索所有会话正文" aria-label="搜索所有会话正文" aria-pressed={Boolean(viewModel.historySearchResults)}><FileSearch size={14} /></button> : null}{viewModel.historySearchResults && view === "directory" ? <button type="button" className="bare-button" onClick={() => { setSearch(""); actions.onSearchHistory(""); }} title="清除正文搜索" aria-label="清除正文搜索"><X size={12} /></button> : null}</div>
    <div className="history-filter-row">
      <div className="history-view-switch" role="tablist" aria-label="会话视图">
        <button type="button" role="tab" className={view === "directory" ? "active" : ""} aria-selected={view === "directory"} onClick={() => selectView("directory")}><FolderOpen size={12} /><span className="history-view-label-wide">当前目录</span><span className="history-view-label-narrow">当前</span></button>
        <button type="button" role="tab" className={view === "favorites" ? "active" : ""} aria-selected={view === "favorites"} onClick={() => selectView("favorites")}><Star size={12} fill={view === "favorites" ? "currentColor" : "none"} /><span className="history-view-label-wide">已收藏</span><span className="history-view-label-narrow">收藏</span></button>
      </div>
    ...
    <nav className="thread-list" aria-label={view === "favorites" ? "已收藏会话列表" : "当前目录会话列表"}>{visible.length ? <>
      {visible.map((entry) => {
        ...
        return <div className="thread-row" key={`${entry.provider}:${entry.id}`}><button className={`thread-item ${active ? "active" : ""}`} aria-current={active ? "page" : undefined} onClick={() => actions.onOpenHistory(entry)} onContextMenu={(event) => { event.preventDefault(); setContextMenu({ entry, x: event.clientX, y: event.clientY, working }); }} title={entry.title}>
          <ProviderIcon provider={entry.provider} size={14} />{marker ? <span className="thread-item-marker">{marker}</span> : null}<span className="thread-item-text"><span className="thread-item-title">{entry.title}</span>{viewModel.historySearchResults && entry.searchSnippet ? <small className="thread-search-snippet">{entry.searchSnippet}</small> : null}{view === "favorites" || viewModel.historySearchResults ? <small className="thread-item-directory"><FolderOpen size={10} />{basename(entry.cwd)}</small> : null}</span><span className="thread-item-time">{formatRelativeTime(Math.max(entry.updatedAt, viewModel.liveThreadActivity[`${entry.provider}:${entry.id}`] || 0))}</span>{entry.isFavorite ? <Star className="thread-item-favorite" size={12} fill="currentColor" aria-label="已收藏" /> : null}
        </button></div>;
      })}
    ...
    </> : <div className="empty-thread"><span className="sidebar-copy">{search.trim() ? "没有匹配会话" : view === "favorites" ? "暂无收藏会话" : "当前目录暂无会话"}</span></div>}{view === "directory" && viewModel.historySearchResults && viewModel.historySearchHasMore ? <button className="history-more" onClick={actions.onLoadMoreHistorySearch} disabled={viewModel.historySearchLoading}>{viewModel.historySearchLoading ? "正在加载更多结果" : "加载更多搜索结果"}</button> : view === "directory" && viewModel.historyHasMore && !viewModel.historySearchResults ? <button className="history-more" onClick={actions.onLoadMoreHistory} disabled={viewModel.historyLoading}>{viewModel.historyLoading ? "正在加载更早会话" : "加载更早会话"}</button> : null}</nav>
