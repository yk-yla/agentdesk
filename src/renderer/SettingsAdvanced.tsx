  ...
  return (
    <>
      <section className="settings-terminal-config">
        ...
        <div className="settings-terminal-primary">
          <label className="settings-inline-field"><span>终端程序</span><select value={kind} disabled={saving} onChange={(event) => selectTerminal(event.target.value as ExternalTerminalKind)}>{EXTERNAL_TERMINAL_PRESETS.map((preset) => <option value={preset.id} key={preset.id}>{preset.label}</option>)}<option value="custom">自定义终端</option></select></label>
